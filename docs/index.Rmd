...
O dataset `rdchem` contém informação de 32 empresas da industria química. Entre as in-
...
rdchem_fit = rdchem_fit.fit()

# print regression table:
table = pd.DataFrame({'Estimate': round(rdchem_fit.params, 4),
                      'Std. Error': round(rdchem_fit.bse, 4),
                      't value': round(rdchem_fit.tvalues, 4),
                      'Pr(>|t|)': round(rdchem_fit.pvalues, 4)})
...
```{python}
# CV para alfa = 5% e 1% usando a distribuição t com n-k-1 d.f.:
n = len(rdchem_pyt)
k = 2
nk_1 = n-k-1
...
#cv_t = stats.t.ppf(1 - alpha / 2, nk_1)
#print(f'cv_t: {cv_t}\n')

# CV for alpha=5% using the normal approximation:
#cv_n = stats.norm.ppf(1 - alpha / 2)
#print(f'cv_n: {cv_n}\n')

rdchem_fit_r2 = rdchem_fit.rsquared
print(f'rdchem_fit_r2: {rdchem_fit_r2}\n')


# F statistic:
#fstat = (rdchem_fit_r2 - r2_r) / (1 - rdchem_fit_r2) * (n - 6) / 3
#print(f'fstat: {fstat}\n')
